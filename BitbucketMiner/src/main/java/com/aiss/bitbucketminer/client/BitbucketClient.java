...
public class BitbucketClient {
    ...
    public List<CommitDto> getCommits(String workspace, String repoSlug, int pageSize, int maxPages) {
        ...
        for (var c : raws) {
            ...
            dto.setTitle(rawSum != null
                    ? rawSum.split("\n",2)[0]
                    : (msg == null ? null : msg.split("\n",2)[0]));
        ...
        while (next != null) {
            try {
                ...
                for (var c : (List<Map<String,Object>>) body.get("values")) {
                    // Extraer el texto bruto del comentario
                    Map<String,Object> content = (Map<String,Object>) c.get("content");
                    String rawBody = content == null ? null : (String) content.get("raw");

                    // Filtro: si está nulo o en blanco, lo ignoro
